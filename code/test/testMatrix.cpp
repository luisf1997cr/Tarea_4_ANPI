...
void testMultiply()
{
  ...
  { //Matrix vector multiplication
    M a = {{1, 2, 3}, {4, 5, 6}};
    std::vector<typename M::value_type> d = {2, 2, 2};
    std::vector<typename M::value_type> expectedResult = {12, 30};

    std::vector<typename M::value_type> providedResult = a * d;

    BOOST_CHECK(providedResult == expectedResult);
    //write more test for vector multiplication
  }
  ...
  { // matrix multiplication
    ...
    M mult_a_r = {{12, 12}, {30, 30}};

    M res = a * r;

    BOOST_CHECK(res == mult_a_r);

    //write more tests for matrix multiplication
  }
}

BOOST_AUTO_TEST_CASE(Multiply)
{
  dispatchTest(testMultiply);
}
