...
void Matrix<T, Alloc>::_create_storage(size_t __rows, size_t __cols)
{
  ...
  { // do not align the rows, just the complete memory block

    // total number of blocks
    const size_t blocks = (__cols * __rows * sizeof(T) + (_Matrix_impl::alignment - 1)) /
                          _Matrix_impl::alignment;
                           ...
                           const Matrix<T, Alloc> &b)
{
  ...
  if (a.cols() != b.rows())
  {
    throw anpi::Exception("amount of columns of the first matrix must be equal to the amount of rows of the second matrix");
  }
                         ...
                         const std::vector<T> &b)
{
  ...
  if (a.cols() != b.size())
  {
    throw anpi::Exception("size of vector must be equal to the size of columns");
  }
